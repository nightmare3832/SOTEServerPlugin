...
public class BedwarsSignManager{
    ...
    public BedwarsSignManager(){
        Server.getInstance().loadLevel("bedwars");
    }
    ...
    public static void updataSign(int number){
        ...
        else updataSign(number,Server.getInstance().getLevelByName("lobby"));
    }
    ...
    public static void updataSign(int number,Level level){
        ...
        for(Map.Entry<Long,BlockEntity> e : level.getBlockEntities().entrySet()){
            ...
            if(blockentity instanceof BlockEntitySign){
                ...
                String[] md = lines.split(GameProvider.BEDWARS);
                if(md.length >= 2){
    ...
    public static String[] getText(int number){
        ...
        Game game = GameProvider.Games.get(number).get(GameProvider.BEDWARS);
            if(game.getGameDataAsBoolean("gamenow")){
                ...
                text[1] = "§0Voting";
                text[2] = "§a"+game.getGameDataAsInt("count")+"/"+Bedwars.MAX_PLAYERS;
            }
        text[3] = "§0> "+GameProvider.BEDWARS+getStringNumber(number)+" <";
        return text;
    }
