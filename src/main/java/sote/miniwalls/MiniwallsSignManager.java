...
public class MiniwallsSignManager{

    public MiniwallsSignManager(){
    ...
    public static void updataSign(int number){
        ...
        else updataSign(number,Server.getInstance().getLevelByName("walls"));
    }
    ...
    public static void updataSign(int number,Level level){
        ...
        for(Map.Entry<Long,BlockEntity> e : level.getBlockEntities().entrySet()){
            ...
            if(blockentity instanceof BlockEntitySign){
                ...
                String[] md = lines.split(GameProvider.MINIWALLS);
                if(md.length >= 2){
    ...
    public static String[] getText(int number){
        ...
        Game game = GameProvider.Games.get(number).get(GameProvider.MINIWALLS);
            if(game.getGameDataAsBoolean("gamenow2")){
                ...
                if(game.getGameDataAsBoolean("timenow")){
                    text[0] = "§aPLAY §0"+game.getGameDataAsBoolean("time")+"s";
                }else{
                    text[0] = "§aPLAY";
                }
                text[1] = "§0Waiting";
                text[2] = "§a"+game.getGameDataAsInt("count")+"/"+Miniwalls.MAX_PLAYERS;
            }
        text[3] = "§0> "+GameProvider.MINIWALLS+getStringNumber(number)+" <";
        return text;
    }
